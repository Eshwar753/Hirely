...
import { useUpdateJobMutation } from "../slices/jobsApiSlice";
...
const PostJobScreen = () => {
  ...
  const  {jobDetails } = useSelector((state) => state.postedJobs);
  ...
  useEffect(() => {
    ...
    setMinimumQualification(job.minimumQualification);
    setSkillsRequired(job.skillsRequired);
    setSmallDescription(job.smallDescription);
    ...
    setEmploymentStatus(job.employmentStatus);
  }, [job]);
